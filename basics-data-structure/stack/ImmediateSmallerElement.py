...
def ImmediateSmallerElement(arr):
    newLst = []
    for i in range(len(arr) - 1):
        immediateVal = arr[i]
        if immediateVal > arr[i+1]:
            immediateVal = arr[i+1]
            newLst.append(immediateVal)
        else:
            newLst.append(-1)
    newLst.append(-1)
    return newLst
...
print(ImmediateSmallerElement([4, 2 ,1 ,5 ,3]))
print(ImmediateSmallerElement([5, 6, 2 ,3 ,1, 7]))
